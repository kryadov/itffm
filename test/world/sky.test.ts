...
describe('buildSky', () => {
  ...
  it('follows the camera so the player never reaches its edge', () => {
    ...
    sky.update(pos, 0xa8c0a2, 0xfff1cf, new THREE.Vector3(1, 2, 1), 1, 0, 0.5)
    expect(sky.mesh.position.equals(pos)).toBe(true)
  })

  it('applies the colours, sun direction and moon phase it is given', () => {
    const sky = buildSky()
    const dir = new THREE.Vector3(1, 2, 1)
    sky.update(new THREE.Vector3(), 0xa8c0a2, 0xfff1cf, dir, 0.5, 0.3, 0.75)
    const mat = sky.mesh.material as THREE.ShaderMaterial
    ...
    expect(mat.uniforms.uMoonPhase.value).toBe(0.75)
