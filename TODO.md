...
- [x] **Фон леса** — ветер в кронах. Синтез (`audio/windAmbience.ts`'s чистая
      `windGain(weather, night)` + `AudioEngine.updateWindAmbience` в
      `audio/audio.ts`), не запись — в отличие от музыки/шагов, тут не было
      живого запроса на конкретную запись, а исключение "не CC0-запись"
      остаётся исключением, не новым умолчанием (см. докстринг
      `AudioEngine`). Тот же приём, что уже у воды: зацикленный шум через
      lowpass с одним LFO, качающим срез частоты (~раз в 14с — "порыв").
      Меняется от погоды и времени суток, как и просил пункт: дождь/снег
      звучат ветренее ясной погоды, туман — тише всех (сырой неподвижный
      воздух), ночь добавляет громкости поверх любой погоды (`NIGHT_BOOST`).
      Не скрип стволов/шорох листвы отдельно — один непрерывный слой ветра,
      этого достаточно, чтобы фон не звучал одной мёртвой петлёй; отдельные
      дискретные скрипы/шорохи можно добавить отдельным заходом, если
      мало. Заодно нашёл и исправил флейки-тест (`test/save/store.test.ts`
      сравнивал `mergeSave(undefined)` с отдельным вызовом `emptySave()`
      через `toEqual` — оба содержат `Date.now()` в новом `calendarStart`,
      разные миллисекунды ломали сравнение). `npm test` (2258 тестов) /
      `build`/`boot-check` зелёные.
