...
- [x] **Звук ходьбы ужасен, звук бега хороший — сделать ходьбу из бега.**
      Живой запрос. `walk.mp3` был отдельной, откровенно хуже звучащей
      записью; `run.mp3` — той же породы звук, но заметно чище. Убрали
      `walk.mp3` целиком (файл, загрузка, поле класса) — оба темпа теперь
      берут один и тот же `run.mp3`, отличаясь параметрами воспроизведения:
      `footstepClip()` получил `FOOTSTEP_CLIP_PARAMS` с `offset`/`duration`/
      `rate`/`gain` на каждый темп. Бег — прежний быстрый кусок у самого
      начала. Ходьба — другой изолированный чистый удар из середины клипа
      (найден тем же способом, что нашёл баг тишины в прошлом фиксе —
      `ffmpeg silencedetect`, звук 0.202-0.554с, тишина по обе стороны), но
      воспроизводится с `playbackRate` 0.8 — ниже по тону и растянут во
      времени, ближе к неспешному шагу, чем к тому же звуку бега просто
      потише. `AudioBufferSourceNode.start(when, offset, duration)`'s
      `offset`/`duration` не зависят от `playbackRate` (время буфера, не
      реальное) — код это учитывает отдельно для огибающей громкости
      (`realDuration = duration / rate`), чтобы не щёлкало на обрезке.
      `npm test`/`build`/`boot-check` зелёные; вживую на слух — не
      проверяемо в headless (нет вывода звука), тот же класс честно
      принятого ограничения, что и у остального звука в этом файле.
- [x] **Окошки в домике очень маленькие и непрозрачные.** Живой репорт. Оба
      верны: рама была 0.5×0.5м на 2.6-метровой стене (мелкий иллюминатор,
      не окно), а `glassMat` (`MeshStandardMaterial`) вообще не выставлял
      `transparent`/`opacity` — по умолчанию это полностью непрозрачный
      материал, так что "стекло" было просто цветным квадратом, который по
      ночам светился, и всё. Исправлено: рама 0.85×0.95м, стекло 0.71×0.81м
      (почти вдвое крупнее по каждой оси), `transparent: true, opacity: 0.4`
      — не идеально прозрачно (настоящее оконное стекло тоже не бывает),
      но читается как стекло, а не дыра в стене. Рейки-перемычки выросли
      пропорционально. Новый тест проверяет `transparent`/`opacity` стекла.
      `npm test`/`build`/`boot-check` зелёные.
- [x] **Звуки шагов при ходьбе пропали совсем (тишина).** Живой репорт
      сразу после v0.80.1's фикса синхронизации. Настоящая причина, найдена
      численно (`ffmpeg silencedetect`), не на глаз: у `public/audio/
      walk.mp3` первые 0.52с — чистая тишина (естественный подвод в
      исходной записи), а `AudioEngine.footstepClip()` всегда проигрывает
      ровно первые `FOOTSTEP_CLIP_DURATION.walk` (0.4с) от начала буфера —
      то есть на каждый шаг честно звучала тишина, а не баг тишины вообще
      (`run.mp3` не пострадал — там подвод всего 0.06с, короче окна
      воспроизведения). Исправлено перетримом `walk.mp3` (`ffmpeg -ss 0.49`)
      так, чтобы offset 0 совпадал с реальным началом звука шага, тем же
      способом очистки, что и раньше — без изменения кода
      (`footstepClip()` и так корректно проигрывал ровно то, что ему дали).
      Проверено численно (`silencedetect` больше не находит тишину в начале
      файла) — вживую доиграть звук нельзя headless-браузером
      (`AudioContext` не выводит на колонки в CI), тот же класс ограничения,
      что уже принят для остального звука в этом файле.
- [x] **Дерево — ствол с крошечной кроной не на верхушке, не похоже на
      дерево.** Живой репорт. Настоящий баг в `world/trees.ts`: ветка
      хвойных верно умножает радиус кроны на `look.crown` (радиус в метрах,
      свой на вид — 2.4 у берёзы, 3.4 у дуба), а ветка лиственных — НЕТ,
      совсем не использует `look.crown`, только "spread" (0.75-1.25,
      зависит от доли роста внутри диапазона высот вида) — то есть крона
      любого лиственного дерева получала одинаковый ~1-метровый радиус
      независимо от вида, отсюда и "тонкая палка с шариком". Исправлено:
      добавлено умножение на `look.crown` в лиственную ветку, тем же
      способом, что уже верно работает у хвойных. Радиус кроны вырос в
      2-4 раза (реалистичные 1.8-4м вместо ~1м), крона стала заметно ближе
      к верхушке ствола (не идеально вплотную — честный компромисс, полное
      "крона строго на верхушке" потребовало бы отдельной перепривязки
      высоты кроны к её собственному радиусу, не только к доле высоты).
      Новый тест: сравнивает масштаб кроны берёзы и дуба одной высоты — до
      фикса они совпадали, после — дуб заметно шире. `npm test`/`build`/
      `boot-check` зелёные.
...
- [x] **Рельсы оторваны от шпал и рельефа.** Живой репорт со скриншотом.
      Настоящая причина — не та же, что у прошлого рельсового бага ниже:
      каждый рельс строился ОДНИМ жёстким `BoxGeometry` на всю длину линии,
      позиция по высоте бралась ОДНИМ замером `railHeightAt` ровно в
      середине линии — плоский рельс поверх шпал, которые (уже тогда)
      честно следовали рельефу в каждой точке. На ровной демо-земле
      разница не бросалась в глаза, на настоящем склоне рельс уходит в
      воздух на одном конце и в землю на другом. Исправлено: каждый рельс
      теперь цепочка коротких сегментов (по паре соседних точек линии, тех
      же `RAIL_STEP`-сэмплов, что уже используют шпалы и поезд через
      `railHeightAt`), каждый повёрнут под свой локальный уклон и слит в
      один меш на сторону (`mergeGeometries`) — по-прежнему два вызова
      отрисовки, не по одному на сегмент. Новый тест на синтетическом
      склоне (0.3 уклон) проверяет, что рельс держит реальный перепад
      высоты по всей длине, а не сглаживается до плоского. `npm test`
      (2254)/`build`/`boot-check` зелёные.
...
- [x] **Живой репорт после релиза календаря: «грибы фактически перестали
      встречаться».** Настоящая причина — не баг в смысле неверного кода,
      а честный побочный эффект: `gameDaysElapsed` всегда стартует с
      игрового месяца 1, а у подавляющего большинства видов `season` —
      реальные весна-осень (4-11, зиму захватывают единицы). Сохранение,
      созданное в сентябре, часами (реально — сутками, месяц двигается раз
      в 30 реальных часов) торчало в "зиме", где почти ничему нельзя было
      расти. Исправлено в два шага: (1) `world/calendar.ts`'s новая
      `realMonthAt(ms)` + `main.ts` теперь добавляет смещение
      `(realMonthAt(save.calendarStart) - 1) * DAYS_PER_MONTH` к
      `gameDaysElapsed`, так что игровой месяц 1 = реальный месяц, в
      котором сохранение реально появилось, а не всегда январь. (2) Второй
      живой запрос сразу следом: «нужно чтобы грибы присутствовали всегда
      вне зависимости от месяца» — `ecology/spawn.ts`'s `speciesScore`
      раньше жёстко обнулял вид не в сезон (`return 0`), теперь это крутой,
      но не абсолютный вес (`OFF_SEASON_WEIGHT = 0.08`) — вне сезона вид
      редок, но не исчезает начисто, так что ускоренный календарь больше
      не может опустошить лес целиком на весь игровой месяц. Тест
      `scores zero out of season` переписан в `scores much lower out of
      season, but never zero`. `npm test` (2256)/`build`/`boot-check`
      зелёные.
- [x] **Сезоны/календарь.** Решено в брейншторме 2026-09-11, реализовано
      позже той же категорией "решение уже принято, доделать самостоятельно".
      Новый чистый модуль `world/calendar.ts`: `gameDaysElapsed(startMs,
      nowMs)` — 1 реальный час = 1 игровые сутки (`REAL_HOURS_PER_GAME_DAY`);
      `gameMonth(gameDays)` — 360-дневный год (30 дней/месяц), не привязан к
      реальному календарю, так что окно `season` любого вида приходит раз в
      ~15 реальных дней, а не раз в год; `isRainDay(seed, dayIndex)` и
      `daysSinceRain(seed, gameDays)` — детерминированная история дождей по
      сиду (тот же принцип, что весь проект — без `Math.random`), с
      ограничением поиска в 14 дней (за этим порогом штраф засухи в
      `speciesScore` уже сошёл на нет). `SaveData.calendarStart` (новое поле,
      `Date.now()` при первом создании сохранения) — фиксированная точка
      отсчёта, переживающая перезагрузку страницы; `main.ts` считает
      `gameDays` один раз при загрузке и пробрасывает в `createForest`/
      `createWorldStream` (оба получили новый опциональный параметр
      `gameDays = 0`, старые вызовы/тесты не сломаны). `ecology/spawn.ts`'s
      `SpawnContext.month`/`daysSinceRain` в обоих местах спавна (домашний
      участок и стримингуемые чанки) теперь читают эти значения вместо
      `new Date().getMonth()+1` (реальная дата) и константы `2`.
      **Сознательно НЕ сделано в этот заход**: видимая погода
      (`world/weather.ts`) не привязана автоматически к тому же значению —
      настройка в меню (`Prefs.weather`) остаётся ручным, фиксированным
      выбором игрока. Автоматическая привязка без отдельного режима "авто" в
      UI тихо перебивала бы явный выбор игрока без всякого объяснения —
      решение по интерфейсу, которое стоит обсудить с пользователем, не
      разворачивать в одиночку. Закрывает пункт «Погода как выбор игрока»
      ниже лишь частично (даты/засуха живут от календаря, видимая погода —
      ещё нет). Новые тесты (`test/world/calendar.test.ts`, 13 штук) —
      детерминизм, границы месяца/года, монотонность засухи. `npm test`/
      `build`/`boot-check` зелёные.
...
- [ ] **Погода как выбор игрока, а не следствие `daysSinceRain`.**
      Наполовину закрыто (см. «Сезоны/календарь» выше, v0.85.0): `ecology/
      spawn.ts`'s `daysSinceRain` больше не константа `2`, а живёт от
      игрового календаря (`world/calendar.ts`). Видимая погода
      (`world/weather.ts`, настройки — `M`) по-прежнему независимая ручка —
      сознательно не тронута, решение по UI (нужен ли отдельный режим
      "авто"), не разворачивать в одиночку.
...
- [ ] **Фон леса** — ветер в кронах, скрип стволов, шорох листвы. Меняется от
      погоды и времени суток, а не крутится одной петлёй. **Попытка и
      откат (v0.86.0 → v0.87.0):** синтезированный ветер (зацикленный шум
      через lowpass с одним LFO, ~14с период) вышел в релиз, живой репорт
      сразу же — «навязчивый/повторяющийся». Один короткий (2с) шумовой
      буфер и один LFO с фиксированным периодом были САМОЙ причиной: ухо
      быстро цепляется за точку повтора короткой петли, и с одной
      гармоникой сверху это читается как механический цикл, а не ветер.
      Убрано целиком по прямой просьбе пользователя, а не приглушено —
      `audio/windAmbience.ts`, `AudioEngine.updateWindAmbience`/
      `ensureWindAmbience`, вызов в `main.ts` удалены. Если возвращаться к
      этому пункту снова, для настоящей неповторяющейся текстуры нужно
      заметно длиннее буфер (секунд 20-30+, не 2) и минимум два LFO на
      некратных периодах (частота И амплитуда порознь, как у воды, а не
      только частота) — то, что не было сделано в первой попытке. Заодно
      нашёл и оставил в силе фикс флейки-теста (`test/save/store.test.ts`
      сравнивал `mergeSave(undefined)` с отдельным вызовом `emptySave()`
      через `toEqual` — оба содержат `Date.now()` в `calendarStart`,
      разные миллисекунды ломали сравнение) — сам баг не связан со звуком
      ветра, но был найден при той же работе и чинить его заново незачем.
...
- [x] **Сезоны и календарь.** Решение принято в брейншторме 2026-09-11 (сам
      идёт, 1 реальный час = 1 игровые сутки — календарь не ждёт, пока игрок
      попросит "подождать") и реализовано — см. «Сезоны/календарь» в
      «🐞 Живой репорт 2026-09-11» выше (`world/calendar.ts`), v0.85.0.
