...
- [ ] **Настройка мыши — инвертировать обзор по вертикали.** Пункт в меню
      настроек (`ui/settingsMenu.ts`, рядом с уже существующей чувствительностью
      мыши), новый флаг в `Prefs`. Меняет знак у `dPitch` — `game/controls.ts`'s
      `move()` (`dPitch -= e.movementY * ...`) и `game/touchControls.ts`'s
      свайп-обзор используют один и тот же расчёт, оба должны учитывать флаг
      одинаково.
