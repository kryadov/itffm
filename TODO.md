...
- [x] **Тот же конус на практике почти никогда не срабатывал для ягод.**
      ...
      каждой ягоды. Исправлено (v0.46.0) радикальнее: конус пробуется
      всегда, как только точный луч не попал ИМЕННО в цель — свойство
      «спрятан за кустом остаётся спрятан» больше не нужно эмулировать
      отдельно, потому что обычный (крупный) гриб никогда не попадает в
      `smallObjects` и через конус просто не ищется.
...
- [ ] **Ягоды визуально не похожи на настоящие.** По просьбе пользователя
      2026-09-09: ягода должна расти на кусте/ветке/кустарнике, а не
      просто висеть кластером сфер в воздухе — и число ягод в грозди
      зависит от вида (черника — по многу ягод на кусте, другие виды
      иначе). `berry/build.ts` сейчас — `InstancedMesh` сфер вокруг
      центра + один конус-лист, без самого куста-носителя. Нужна
      морфология под конкретный вид (куст с листвой для черники/брусники,
      возможно другая форма для земляники — она не кустовая) — вероятно
      стоит брейншторма по объёму (несколько видов, разные "растёт как":
      кустарник/трава/стелющееся), не переписывать `build.ts` с ходу.
...
- [ ] **Землянка — доработка по живому фидбеку 2026-09-09.** Три отдельные
      правки: (1) окна выглядят ненатурально — нужны настоящие рамы
      (переплёт), а не голая рамка+стекло; (2) из-под дома снизу протекает
      свет — убрать (похоже, `PointLight` очага или ночная подсветка окна
      бьёт через пол/стены геометрии сруба, а не только наружу); (3) рядом
      с домом положить охапку дров (заодно субстрат для мха/`deadwood`?) и
      колодец с ведром — два новых декоративных объекта у землянки,
      наподобие того как уже стоит труба/дверь/окна.
