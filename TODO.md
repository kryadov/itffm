...
- [ ] **Живность (2/3): змея.** Третье поведение по месту — "медленно
      уползает" вместо испуга вскачь (решено в брейншторме: свой переход
      в `flee` с низкой `fleeSpeed` и меньшим `fleeRadius`, та же state
      machine из `world/critters.ts`, не новый файл). Поза — цепочка
      сегментов вдоль кривой пути, а не heading одного тела — единственное,
      что здесь по-настоящему ново относительно зайца/белки.
- [ ] **Живность (3/3): улей + пчёлы + стрекозы.** `world/insects.ts` —
      рой без состояний (кружит вокруг опорной точки, не реагирует на
      игрока), `world/hive.ts` — дикий улей в дупле/пне у случайного
      дерева (решено в брейншторме, не отдельно стоящий ящик и не пасека).
      Стрекозы садятся по `classifyWater(ring) === 'pond'` — только когда в
      локации вообще есть вода, тот же паттерн, что у домика рыбака
      (`fisherHut.ts`, возвращает `null`, если водоёма нет).
