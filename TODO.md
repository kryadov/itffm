...
- [ ] **Нет подсказок по клавишам во время ходьбы, нет экранных кнопок.**
      Замечено пользователем 2026-09-08: подсказка по управлению есть только
      один раз, на экране disclaimer (`controls` в `i18n.ts`), после — негде
      вспомнить, что открывает корзину/настройки/энциклопедию. Нужна
      постоянная (или по нажатию отдельной клавиши, напр. `H`) панель-
      подсказка, и — раз уж мобильные кнопки всё равно в планах (см.
      «Мобильные устройства» ниже) — их разумно проектировать одним заходом:
      те же самые кнопки, которые появляются на мобильном тач-экране, могут
      быть видны (полупрозрачно) и на ПК как подсказка, а не два отдельных
      UI.
...
- [ ] **Ручей, водопад, источники рядом с водоёмами.** Сейчас `world/water.ts`
      рисует только стоячие пруды/озёра и прямые русла рек по OSM-полигонам
      (`geo/parse.ts`) — ни течения, ни перепада высоты вода не знает. Нужны:
      течение/рябь на ручьях, водопад там, где русло пересекает резкий
      перепад рельефа, и родники/источники — маленькие, отдельно стоящие,
      не обязательно на трассе реки по OSM.
