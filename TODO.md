...
- [x] **Тот же конус на практике почти никогда не срабатывал для ягод.**
      Готово (v0.44.0), живой баг-репорт: «с грибами всё работает, а на
      ягоды надпись не появляется». Причина — не масштаб, а сама трава
      вокруг: точный луч, задевший ЛЮБОЙ окклюдер (травинку/куст) без
      находки за ним, возвращал `null` и НИКОГДА не пробовал конус —
      предполагалось, что это «гриб спрятан за кустом», но ягода растёт
      прямо В траве, так что эта трава почти всегда оказывалась на луче,
      съедая запасной проход именно там, где он нужнее всего. Тест
      `test/game/pick.test.ts`, добавленный вместе с самим прощающим
      конусом, закреплял именно эту ошибку как «правильное» поведение.
      Исправлено: конус пробуется всегда, если на всём луче (не только
      ближайшем попадании) нет ни одной настоящей находки — гриб, реально
      стоящий ЗА кустом на том же луче, по-прежнему остаётся скрытым.
